...
export class Hex {
    ...
    public static direction(direction: Direction): Hex {
        return Hex.directions[direction];
    }


    public neighbor(direction: Direction): Hex {
        return this.add(Hex.direction(direction));
    }

    public static diagonals: Hex[] = [new Hex(2, -1, -1), new Hex(1, -2, 1), new Hex(-1, -1, 2), new Hex(-2, 1, 1), new Hex(-1, 2, -1), new Hex(1, 1, -2)];

    public diagonalNeighbor(direction: Direction): Hex {
...
export class DoubledCoord {
    constructor(public col: number, public row: number) {
        if ((col + row) % 2 !== 0) {
            throw Error('col and row sum must be possible to divide by 2 without reminder.');
        }
    }

    toCube(): Hex {
        const x = this.col
        const z = (this.row - this.col) / 2
        const y = -x - z
        return new Hex(x, y, z)
    }

    static fromCube({ q, s}: Hex): DoubledCoord {
        const col = q;
        const row = 2 * s + q;
