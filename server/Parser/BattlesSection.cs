...
// Sandlings (926) loses 8.
public class CasualtiesParser : BaseParser {
    public CasualtiesParser(IParser unitName) {
        this.unitName = unitName;
    }
    ...
    protected override PMaybe<IReportNode> Execute(TextParser p) {
        ...
        var amount = p.After(" loses").SkipWhitespaces().BeforeBackwards(".").Integer();
        if (!amount) return Error(amount);
